...
4. Completed: strengthened the P11 live DeepSeek acceptance test so it uses a natural business question instead of a SQL-shaped prompt.
5. Next: clean current docs so Streamlit, ecommerce eval, mock Jira/Power BI, and old demo references are historical only.
...
- P10 external-safe MCP contract metadata plus artifact hygiene coverage for generated DBs, eval reports, traces, charts, workbooks, and mock external outputs
...
By default the eval writes its Markdown report to `eval/report.md` and traces to `logs/traces/eval/`. Use `--report-path` and `--trace-dir` to redirect those outputs for local runs. Action eval DBs are written under the selected trace directory; visualization charts, XLSX exports, and mock external artifact URLs are runtime outputs. Generated eval reports, traces, charts, workbooks, action DBs, and mock artifacts should not be committed.
