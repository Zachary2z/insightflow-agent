...
def test_provider_runner_returns_json_compatible_trace_metadata():
    ...
    provider = MockLLMProvider({"sql_candidates": [{"sql": "SELECT 1", "rationale": "baseline"}]})
    request = LLMRequest(
        prompt="Return guarded SQL candidates.",
        prompt_id="guarded_sql_candidate",
        prompt_version="v1",
        model="mock-free",
        metadata={"node": "sql_candidate_agent"},
    )

    result = run_llm_request(provider, request)

    assert result["success"] is True
    assert result["content"] == {"sql_candidates": [{"sql": "SELECT 1", "rationale": "baseline"}]}
    ...
    assert result["trace_event"]["prompt_id"] == "guarded_sql_candidate"
