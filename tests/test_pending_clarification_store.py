...
def test_pending_clarification_store_supports_running_and_failed_states(tmp_path):
    from workspaces.pending_clarification_store import PendingClarificationStore

    workspace_store = WorkspaceStore(tmp_path / "workspaces")
    workspace = workspace_store.create_workspace("Pending Status Workspace")
    store = PendingClarificationStore(workspace_store)
    pending = store.create_pending_run(
        workspace_id=workspace["workspace_id"],
        run_id="run_1",
    ...
    running = store.mark_running(
        ...
        pending_run_id=pending["pending_run_id"],
        clarification_answer="最近 90 天",
        resolved_question="帮我看看销售情况。补充回答：最近 90 天。",
    )
    ...
    assert failed["error"] == "workflow exploded"
