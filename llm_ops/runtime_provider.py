...
def provider_claim_typing_enabled(env: dict[str, str] | None = None) -> bool:
    values = env if env is not None else os.environ
    return str(values.get("INSIGHTFLOW_USE_PROVIDER_CLAIM_TYPING", "")).strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def provider_action_drafter_enabled(env: dict[str, str] | None = None) -> bool:
    values = env if env is not None else os.environ
    return str(values.get("INSIGHTFLOW_USE_PROVIDER_ACTION_DRAFTER", "")).strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def provider_analysis_planner_enabled(env: dict[str, str] | None = None) -> bool:
    values = env if env is not None else os.environ
    return str(values.get("INSIGHTFLOW_USE_PROVIDER_ANALYSIS_PLANNER", "")).strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def provider_visualization_planner_enabled(env: dict[str, str] | None = None) -> bool:
    values = env if env is not None else os.environ
    return str(values.get("INSIGHTFLOW_USE_PROVIDER_VISUALIZATION_PLANNER", "")).strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
