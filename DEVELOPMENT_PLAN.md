...
Live DeepSeek tests remain opt-in. To run live acceptance locally, set `INSIGHTFLOW_LIVE_DEEPSEEK_TESTS=1`, `INSIGHTFLOW_PRODUCT_LIVE_MODE=1`, and `DEEPSEEK_API_KEY`, then run the current live analysis/report acceptance tests. Without those flags and a key, live tests skip and normal regression remains deterministic. P23-H6 did not run live provider acceptance in this environment because `DEEPSEEK_API_KEY` and the live flags were absent.
