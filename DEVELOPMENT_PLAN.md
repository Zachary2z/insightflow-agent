...
- P22 evidence-driven report generation work has started. H1 replaced stitched Analysis Workbench report sections with a clean report document contract, then closeout removed the legacy report supervisor/agent/writer/planner files and their old provider prompt/schema flags. H2 added Chinese goal-driven report planning and evidence collection from workspace profile, semantic layer, metric registry, guarded SQL execution, and evidence payload helpers.
...
| P22 | Evidence-driven report generation: replace stitched report sections with a coherent Chinese report document path and delete old report paths | In progress; H1-H2 complete |
